...
local swaysize = 0.75 -- how large/powerful the sway is. Don't go above 2
...
if rigtype == "R15" then
	...
	stepped_con = game:GetService("RunService").RenderStepped:connect(function()
		...
		if isfirstperson == true then
			...
			if isrunning == true and includewalksway and humanoid:GetState() ~= Enum.HumanoidStateType.Freefall and humanoid:GetState() ~= Enum.HumanoidStateType.Landed then
				walksway = walksway:lerp(
					CFrame.new(
						...
						(0.14*swaysize) * math.sin(tick() * (1 * humanoid.WalkSpeed/3)),
						(0.07*swaysize) * math.cos(tick() * (3 * humanoid.WalkSpeed/6)),
						0
						...
						CFrame.Angles(
							...
							0,
							0,
							(-.0275*swaysize) * math.sin(tick() * (2 * humanoid.WalkSpeed/2.65))

						)
					,0.7*sensitivity)
			else
				walksway = walksway:Lerp(CFrame.new(), 0.015*sensitivity)
			end
			--
			local delta = uis:GetMouseDelta()
			if includecamerasway then
				sway = sway:Lerp(Vector3.new(delta.X,delta.Y,delta.X/6), 0.075*sensitivity)
			end
			--
			...
			local finalcf = (camera.CFrame*walksway*jumpsway*strafesway*CFrame.Angles(math.rad(sway.Y*swaysize),math.rad(sway.X*swaysize)/10,math.rad(sway.Z*swaysize)/1.25))+(camera.CFrame.UpVector*(-1.7-headoffset))+camera.CFrame.LookVector*0+(camera.CFrame.RightVector*-(sway.X*swaysize)/25)
			...
			if isrunning == true and includewalksway and humanoid:GetState() ~= Enum.HumanoidStateType.Freefall and humanoid:GetState() ~= Enum.HumanoidStateType.Landed then
				walksway = walksway:lerp(
					CFrame.new(
						...
						(0.14*swaysize) * math.sin(tick() * (1 * 16/3)),
						(0.07*swaysize) * math.cos(tick() * (3 * 16/6)),
						0
						...
						CFrame.Angles(
							...
							0,
							0,
							(-.0275*swaysize) * math.sin(tick() * (2 * 16/2.65))
